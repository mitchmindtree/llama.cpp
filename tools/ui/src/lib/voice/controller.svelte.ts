...
import { type Segment, SentenceChunker } from './chunker';
import { clearDim, type DimState, scheduleDim } from './dimmer';
...
class VoiceController {
	...
	async probe(): Promise<void> {
		this.available =
			...
			Boolean(settingsStore.config.voiceEnabled) &&
		...
		if (this.available) {
			...
			this.speakEnabled = Boolean(settingsStore.config.voiceSpeakReplies);
		}
	}
	...
	private bargeIn(): void {
		if (!settingsStore.config.voiceBargeIn) return;
	...
	private enqueueSegment(seg: Segment): void {
		if (this.trackerStale) {
			...
			this.tracker = {
				convId,
				latex: /\$|\\\(|\\\[/.test(seg.text),
				msgId: convId ? (chatStore.getChatStreaming(convId)?.messageId ?? null) : null,
				segments: []
			};
			this.trackerStale = false;
		}

		const record: TurnSegment = { seg, segId: ++this.segCounter, sent: 0 };
	...
	private halfDuplexGated(): boolean {
		if (!settingsStore.config.voiceHalfDuplex) return false;
	...
	private newChunker(): SentenceChunker {
		...
		return new SentenceChunker(settingsStore.config.voiceParagraphChunks !== false);
	}
	...
	private async pump(): Promise<void> {
		...
		try {
			...
			while (this.queue.length > 0) {
				...
				try {
					for await (const pcm of gateway.speechStream(record.seg.text, voice, this.abort.signal)) {
	...
	private resolveVoice(tag: string | null): string {
		...
		return String(settingsStore.config.voiceVoice || 'narrator');
	}
