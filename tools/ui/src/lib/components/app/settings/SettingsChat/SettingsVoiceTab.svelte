<script lang="ts">
	...
	let { onConfigChange }: { onConfigChange?: (key: string, value: string | boolean) => void } =
		$props();
	...
	function setVoiceConfig(key: string, value: string | number | boolean) {
		...
		onConfigChange?.(key, value as string | boolean);
	}
...
{#if !voiceController.available}
	<div class="py-8 text-center text-sm text-muted-foreground">
		Voice gateway unavailable. Voice needs the HTTPS origin (the mic and audio are disabled on plain
		http) and a reachable gateway.
	</div>
{:else}
	...
	<div class="min-w-0 space-y-8 overflow-hidden">
		...
		<section class="space-y-3">
			...
			<p class="text-xs text-muted-foreground">
				The default voice speaks all replies. A reply segment starting with [voice:name] switches to
				that voice, which lets a DM persona give NPCs their own voices.
			</p>
			...
			<div class="max-h-[26rem] space-y-2 overflow-y-auto pr-1">
				{#each voiceController.voices as voice (voice.name)}
					<div class="flex min-w-0 items-center gap-2 rounded-md border border-border/40 p-2">
						...
						{#if settingsStore.config.voiceVoice === voice.name}
						...
						<Button
							class="h-7 w-7 p-0"
							disabled={settingsStore.config.voiceVoice === voice.name}
			...
			<label class="flex flex-col gap-1 text-sm">
				<span>
					...
					<span class="font-mono"
						>{Number(settingsStore.config.voiceTemperature ?? 0.9).toFixed(2)}</span
					>
				</span>
				...
				<input
					...
					step="0.05"
					type="range"
					value={Number(settingsStore.config.voiceTemperature ?? 0.9)}
				/>

				<span class="text-xs text-muted-foreground">
					Lower is flatter and more stable, higher is livelier with more variation between takes.
					0.90 is the model default.
				</span>
			</label>

			<label class="flex items-center gap-2 text-sm">
				<Checkbox
					checked={settingsStore.config.voiceParagraphChunks !== false}
					onCheckedChange={(v) => setVoiceConfig('voiceParagraphChunks', v === true)}
				/>

				<span>
					Paragraph-scale speech chunks: synthesize several sentences per request for smoother, more
					connected prosody. Off = one sentence per request (earlier behavior, faster reaction to
					very short replies). Applies from the next reply.
				</span>
			</label>

			<label class="flex items-center gap-2 text-sm">
				<Checkbox
					checked={Boolean(settingsStore.config.voiceHalfDuplex)}
					onCheckedChange={(v) => setVoiceConfig('voiceHalfDuplex', v === true)}
				/>

				<span>
					Half-duplex (for speakers): mute mic input while the assistant is speaking. Use when echo
					cancellation cannot stop the assistant from hearing itself. Disables voice barge-in.
				</span>
			</label>
		</section>
		...
		<section class="space-y-3">
			...
			<p class="text-xs text-muted-foreground">
				Record or upload 10-15 seconds of clean speech. The clip's mood carries into everything the
				voice says, so record in the delivery you want (a calm read gives a calm voice, an excited
				read an excited one). Without a transcript, the clip is transcribed automatically (shown
				below after cloning; delete and redo if it is wrong). Voices cloned before the latest
				improvements can be re-cloned to pick them up.
			</p>
			...
			<Input
				bind:value={cloneTranscript}
				placeholder="transcript (optional, auto-transcribed if empty)"
			/>
		...
		<section class="space-y-3">
			...
			<p class="text-xs text-muted-foreground">
				Describe a voice (age, tone, accent, character), audition it on the sample text, then save
